...
def composition_dance():
    ...
    df = pd.read_csv('app/data/composition_prop/dance_net_in.csv')
    net_in = df.values
    ...
    pitchnames = pd.read_csv('app/data/composition_prop/dance_pitchnames.csv')
    ...
    fir.insert(0, instrument.Marimba())
    ...
    tw.insert(0, instrument.PanFlute())
    ...
    thr.insert(0, instrument.Clavichord())
    ...
    for pattern in p4:
        ...
        # pattern이 Note 일 때
            ...
            new_note.offset = offset
            output_notes.append(new_note)
        # 반복마다 ofset을 증가시킨다 (고정 박자)
        offset += 0.3
    fr = stream.Part(output_notes)
    fr.insert(0, instrument.ElectricBass())
