...
def composition(genre):
    model = load_model('app/model/composition_model/rapLSTM_Generator.h5')
    # model.summary()

    # LSTM 모델 입력 생성
    df = pd.read_csv('app/data/composition_prop/rap_net_in.csv')
    net_in = df.values
    ...
    pitchnames = pd.read_csv('app/data/composition_prop/rap_pitchnames.csv')
    ...
    # 음(Note/Chord)을 언제 들려줄지 정하는 timing offset (박자 정보를 대신 함)
    ...
    fir.insert(0, instrument.AcousticGuitar())
    ...
    tw.insert(0, instrument.ElectricOrgan())
    ...
    thr.insert(0, instrument.SteelDrum())
    ...
    fr.insert(0, instrument.Accordion())
    ...
    try:
        ...
        file_name = f'output_{genre}_{str(int(time.time()))}.mid'
